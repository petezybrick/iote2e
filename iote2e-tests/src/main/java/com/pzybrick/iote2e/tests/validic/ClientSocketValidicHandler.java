 ...
 */
public class ClientSocketValidicHandler {
	
	/** The Constant logger. */
	private static final Logger logger = LogManager.getLogger(ClientSocketValidicHandler.class);
	...
	public static void main(String[] args) {
		...
		try {
			ClientSocketValidicHandler clientSocketValidicHandler = new ClientSocketValidicHandler();
			clientSocketValidicHandler.setUrl(args[0]);
			clientSocketValidicHandler.connect();
			clientSocketValidicHandler.wait();
		} catch (Exception e) {
			logger.info(e);
			e.printStackTrace();
		}
	...
	public void connect() throws Exception {
		...
		try {
			...
			try {
				ClientSocketValidic clientSocketValidic = new ClientSocketValidic( );
				session = container.connectToServer(clientSocketValidic, uri);
	 ...
	 */
	public ClientSocketValidicHandler setLoginVo(LoginVo loginVo) {
	 ...
	 * @return the client socket bdbb handler
	 */
	public ClientSocketValidicHandler setUrl(String url) {
		this.url = url;
		return this;
	}
